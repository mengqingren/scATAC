...
# TESTED
# 
# ASSUMES BARCODE IS IN HEADER LINE

if (@ARGV != 7) { 
	print STDERR "perl 1_Flexible_FullTranscript_Demultiplexing.pl read1.fq read2.fq barcode_pos barcode_len index mismatch prefix\n";
	print STDERR "
		read1.fq : barcode containing read
		read2.fq : non-barcode containg read
		barcode_pos : position in read 1 of the barcode (start/end) or if in header
		barcode_len : length of barcode in bases. 
...
#open(my $miss_fh, ">", "Missing_Barcodes.txt") or die "Cannot open Missing_Barcodes.txt\n";
#foreach my $key (keys(%MissingBarcodes)) {
#	if ($MissingBarcodes{$key} >= 1) {
#		print $miss_fh "$key\t".$MissingBarcodes{$key}."\n";
#	}
#}
#close($miss_fh);
